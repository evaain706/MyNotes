...
```
모든 페이지에서 토스트가 나타나게 하기위해서 최상위에 적용
...
1. 유저 상호작용 발생(버튼클릭등에서) `addToast('success', '저장 완료')` 호출
2. store 상태변경 -> 새로운 객체를 `toast`배열에 추가
3. ToastContainer가 `toast`상태를 구독중이기 때문에 리렌더링되며 Toast컴포넌트 렌더링
4. `setTimeout`에 의해 3초후 자동으로 제거
